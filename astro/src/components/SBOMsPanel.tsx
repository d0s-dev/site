...
export function SBOMsPanel({ appId, version, images }: SBOMsPanelProps) {
  ...
  const downloadUrl =
    selectedImages.length === 1 ? getSBOMUrl(appId, version, selectedImages[0]) : undefined;

  const viewerUrl =
    selectedImages.length === 1 ? getSBOMViewerUrl(appId, version, selectedImages[0]) : undefined;
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 grid-cols-3">
        <div className="rounded-lg border border-[#023052] bg-[#001233] p-3 text-center">
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#5C677D]">Packages</p>
          <p className="mt-1 text-xl font-semibold text-white">{loading ? "—" : sbomData.length}</p>
        </div>
        <div className="rounded-lg border border-[#023052] bg-[#001233] p-3 text-center">
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#5C677D]">Types</p>
          <p className="mt-1 text-xl font-semibold text-white">
            {loading ? "—" : new Set(sbomData.map((p) => p.type)).size}
          </p>
        </div>
        <div className="rounded-lg border border-[#023052] bg-[#001233] p-3 text-center">
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#5C677D]">Images</p>
          <p className="mt-1 text-xl font-semibold text-white">{selectedImages.length}</p>
        </div>
      </div>
