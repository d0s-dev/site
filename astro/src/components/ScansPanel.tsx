...
export function ScansPanel({ appId, version, images }: ScansPanelProps) {
  ...
  const downloadUrl =
    selectedImages.length === 1 ? getScanUrl(appId, version, selectedImages[0]) : undefined;
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 grid-cols-4">
        {(["critical", "high", "medium", "low"] as const).map((severity) => {
          const count = cveData.filter((cve) => cve.severity.toLowerCase() === severity).length;
          const colorMap = {
          ...
          return (
            ...
            >
              <p className="text-[10px] uppercase tracking-[0.15em] text-[#5C677D]">{severity}</p>
