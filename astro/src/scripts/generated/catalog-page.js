...
function buildOverlayElements() {
    ...
    return {
        ...
        tabButtons: overlay
            ? Array.from(overlay.querySelectorAll("[data-overlay-tab]"))
            : [],
...
function initFilters() {
    const labelButtons = Array.from(document.querySelectorAll("[data-filter-label]"));
    const providerButtons = Array.from(document.querySelectorAll("[data-filter-provider]"));
...
function renderTabContent(overlayState, elements) {
    const { tabContent, } = elements;
    if (!tabContent)
        return;
    tabContent.innerHTML = "";
    const manifest = overlayState.manifestResult?.manifest;
    const provider = overlayState.providerKey ? manifest?.providers?.[overlayState.providerKey] : undefined;
...
function renderOverlay(overlayState, elements) {
    const { overlayContent, overlayError, overlayErrorMessage, overlayImages, overlayLabels, overlayLatest, overlayLoading, overlayProviders, overlaySource, overlayStatus, overlaySummary, overlayTitle, overlayUpdated, providerSelect, versionSelect, imageList, } = elements;
    ...
    if (!overlayState.providerKey || !manifestResult.manifest.providers?.[overlayState.providerKey]) {
        const preferred = providerEntries.find(([key]) => key === "ironbank")?.[0] ??
...
function attachOverlayHandlers(overlayState, elements, catalogViewModelMap) {
    const { overlay, overlayBackdrop, overlayClose, providerSelect, versionSelect, tabButtons } = elements;
    if (!overlay)
        return;
    const cards = Array.from(document.querySelectorAll("[data-catalog-card]"));
    const resetOverlay = () => {
